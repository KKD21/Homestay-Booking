...
export const isRoomAvailableNow = (reservations) => {
  ...
  return !reservations?.find(
    (reservation) =>
      isToday(reservation.check_in) ||
      isToday(reservation.check_out) ||
      (isBefore(reservation.check_in, new Date()) && isAfter(reservation.check_out, new Date()))
  );
};

export const currentReservationStatus = (reservation) => {
  if (reservation.status === "unconfirmed" && !isAfter(new Date(), reservation.check_in))
    return { type: "warning", status: "Unconfirmed" };
  else if (
    reservation.status === "cancelled" ||
    (reservation.status === "unconfirmed" && isAfter(new Date(), reservation.check_in))
  )
  ...
  } else if (isAfter(new Date(), reservation.check_out)) return { type: "secondary", status: "Passed" };
