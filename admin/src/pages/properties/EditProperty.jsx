...
import { getPropertyById, updateProperty } from "../../services/supabase/properties";
...
function EditProperty() {
  ...
  useEffect(() => {
    if (property) {
      ...
      setValue("bhk", property.bhk || "");
      // Store existing images and thumbnail for reference
      setValue("existingImages", property.images || []);
      setValue("thumbnail", property.thumbnail || "");
      
      // Set amenities if they exist
      if (property.amenities) {
        Object.entries(property.amenities).forEach(([key, value]) => {
          setValue(`amenities.${key}`, value);
        });
      }
    }
  }, [property, setValue]);
  ...
  const { mutate, isPending } = useMutation({
    ...
    mutationFn: async (data) => {
      ...
      // Include existing images in the data
      data.images = data.existingImages || [];
      
      return await updateProperty(id, data, thumbnailFile, imageFiles);
    },
    onSuccess: () => {
  ...
  if (isLoadingProperty) return <LoadingSpinner />;

  return (
    <div className="p-5">
      <h1 className="font-semibold text-2xl mb-7">Edit Property</h1>
      <form action="" onSubmit={handleSubmit(onSubmitForm)}>
        <div className="grid md:grid-cols-2 gap-5">
          ...
          <div className="flex flex-col gap-2">
            ...
            {property?.thumbnail && (
              <div className="mb-2">
                <img 
                  src={`${import.meta.env.VITE_SUPABASE_IMGS_URL}/${property.thumbnail}`} 
            ...
            <div className="mb-2 flex flex-wrap gap-2">
              {property.images.map((img, index) => (
                <div key={index} className="relative">
                  <img 
                    src={`${import.meta.env.VITE_SUPABASE_IMGS_URL}/${img}`} 
                    alt={`Property image ${index + 1}`} 
                    className="w-32 h-24 object-cover"
                  />
                </div>
              ))}
              <p className="w-full text-sm text-gray-500 mt-2">Current images</p>
            </div>
          )}
          <input
        ...
        <div className="mt-5">
          ...
          >
            {isPending ? <LoadingSpinner /> : "Update Property"}
          </button>
        </div>
      </form>
    </div>
