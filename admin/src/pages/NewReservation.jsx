...
function NewReservation() {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: async (data) =>
      await createNewReservation(
        ...
        selectedGuest,
        data.guests_count,
        data.price, // This is the reserved_price parameter
  ...
  function onSubmit(data) {
    ...
    // Validate booking period exists
    if (!data.booking_period || data.booking_period.length < 2) {
      setError("booking_period", { type: "required", message: "Booking period is required!" });
      return;
    }
    
    // Ensure price is converted to a number before submission
    data.price = Number(data.price);
    
    // Ensure booking_period dates are valid Date objects
    if (!(data.booking_period[0] instanceof Date) || !(data.booking_period[1] instanceof Date)) {
      handleArrivalSelect(data.booking_period);
    }
        ...
        <div className="grid md:grid-cols-2 gap-5">
          ...
          <div className="flex flex-col gap-2">
            ...
            <FormSelect
              ...
              options={guests?.map((item) => ({ label: `${item.fullname} - ${item.phone || 'No phone number'}`, value: item.id }))}
            />
          ...
          <div className="flex flex-col gap-2">
            ...
            {errors.price?.type === "min" && (
              <p className="text-sm text-red-700 italic">Price must be greater than zero</p>
            )}
