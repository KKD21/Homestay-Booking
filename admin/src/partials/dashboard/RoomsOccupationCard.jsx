...
function RoomsOccupationCard({ groupedReservations, dateRange, selected_date }) {
  ...
  filteredReservations.forEach((reservation) => {
    ...
    booking_period.forEach((date) => {
      if (
        (isAfter(new Date(date), new Date(reservation.check_in)) &&
          isBefore(new Date(date), new Date(reservation.check_out))) ||
        isEqual(new Date(date), new Date(reservation.check_in)) ||
        isEqual(new Date(date), new Date(reservation.check_out))
      ) {
  ...
  const guestsCountWithinDateRange = Object.values(uniqueDateRangeReservation).reduce((curr, next) => curr + (typeof next === 'number' ? next : parseInt(next) || 0), 0);
