                      ...
                      {headings.find((col) => col.label === "booking range" && col.show) && (
                        <Table.Cell>
                          {item.check_in && item.check_out ? (
                            <>
                              {format(new Date(item.check_in), "LLL dd yyyy")} &ndash; {format(new Date(item.check_out), "LLL dd yyyy")}
                              <br />
                              <span className="italic font-extralight text-slate-500">
                                {differenceInDays(new Date(item.check_out), new Date(item.check_in))} Nights
                              </span>
                            </>
                          ) : (
                            "No dates available"
                      ...
                      {headings.find((col) => col.label === "actions" && col.show) && (
                        <Table.Cell>
                          <DropdownEditMenu align="right" className="relative inline-flex">
                            {!(item.status === "confirmed" && isAfter(new Date(), new Date(item.check_out))) &&
                              item.guest_id && (
                                <li>
                                  <Link
                            ...
                            {(!(item.status === "confirmed" &&
                              isAfter(new Date(), new Date(item.check_in)) &&
                              isBefore(new Date(), new Date(item.check_out))
                            ) ||
                              (item.status === "confirmed" && isAfter(new Date(), new Date(item.check_in)))) && (
                              <li>
                                <DeletionModal
                                  queryKey={"reservations"}
                                  targetName={"The reservation"}
                                  mutationFuntion={async () => await deleteReservation(item.id, !!item.deleted_at)}
                                  modalKey={`reservation-${item.id}-${index}`}
                                />
                              </li>
                            )}
                          </DropdownEditMenu>
